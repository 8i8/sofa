...
var errPlan94 = en.New(1, "Plan96", []string{
	"illegal NP (outside 1-8)",
	"",
	"warning: year outside 1000-3000",
	"warning: failed to converge",
})

//  CgoPlan96 This function is part of the International Astronomical
//  Union's SOFA (Standards Of Fundamental Astronomy) software
//  collection.
//
//  - - - - - - -
//   P l a n 9 4
...
//
//  CgoPlan94 This function is part of the International Astronomical
//  Union's SOFA (Standards Of Fundamental Astronomy) software
//  collection.
func CgoPlan94(date1, date2 float64, np int) (
...
//  GoPlan94 This function is part of the International Astronomical
//  Union's SOFA (Standards Of Fundamental Astronomy) software
//  collection.
func GoPlan94(date1, date2 float64, np int) (
	pv [2][3]float64, err en.ErrNum) {
	...
	// Maximum number of iterations allowed to solve Kepler's
	// equation
	const KMAX = 10
	...
	var amas = [...]float64{6023600.0, // Mercury
		408523.5,  // Venus
		328900.5,  // EMB
		3098710.0, // Mars
		1047.355,  // Jupiter
	...
	// Tables giving the mean Keplerian elements, limited to t^2
	// terms:
	//
	...
	//   omega   longitude of the ascending node (degree and
	//            arcsecond)
	...
	// Tables for trigonometric terms to be added to the mean
	// elements of the semi-major axes
	...
	if (np < 1) || (np > 8) {
		...
		de = e[np][0] +
			(e[np][1]+
				e[np][2]*t)*t
		dp = GoAnpm((3600.0*pi[np][0] +
			(pi[np][1]+
				pi[np][2]*t)*t) * DAS2R)
		di = (3600.0*dinc[np][0] +
			(dinc[np][1]+
				dinc[np][2]*t)*t) * DAS2R
		dom = GoAnpm((3600.0*omega[np][0] +
			(omega[np][1]+
		...
		for k = 0; k < 8; k++ {
			arga = kp[np][k] * dmu
			argl = kq[np][k] * dmu
			da += (ca[np][k]*math.Cos(arga) +
				sa[np][k]*math.Sin(arga)) * 1e-7
			dl += (cl[np][k]*math.Cos(argl) +
				sl[np][k]*math.Sin(argl)) * 1e-7
		}
		arga = kp[np][8] * dmu
		da += t * (ca[np][8]*math.Cos(arga) +
			sa[np][8]*math.Sin(arga)) * 1e-7
		for k = 8; k < 10; k++ {
			argl = kq[np][k] * dmu
			dl += t * (cl[np][k]*math.Cos(argl) +
				sl[np][k]*math.Sin(argl)) * 1e-7
		}
		dl = math.Mod(dl, D2PI)

		// Iterative soln. of Kepler's equation to get eccentric
		// anomaly.
		am = dl - dp
		ae = am + de*math.Sin(am)
		k = 0
		dae = 1.0
		for k < KMAX && math.Abs(dae) > 1e-12 {
			dae = (am - ae + de*math.Sin(ae)) /
				(1.0 - de*math.Cos(ae))
			ae += dae
			k++
			if k == KMAX-1 {
		...
		// True anomaly.
		ae2 = ae / 2.0
		at = 2.0 * math.Atan2(math.Sqrt((1.0+de)/
			(1.0-de))*math.Sin(ae2),
			math.Cos(ae2))
