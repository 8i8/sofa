...
//  CgoNut00b Nutation, IAU 2000B model.
//
//  - - - - - - -
//   N u t 0 0 b
//  - - - - - - -
...
//
//  CgoNut00b Nutation, IAU 2000B model.
func CgoNut00b(date1, date2 float64) (dpsi, deps float64) {
	var cDpsi, cDeps C.double
	C.iauNut00b(C.double(date1), C.double(date2), &cDpsi, &cDeps)
	return float64(cDpsi), float64(cDeps)
}

//  GoNut00b Nutation, IAU 2000B model.
func GoNut00b(date1, date2 float64) (dpsi, deps float64) {
	...
	el = math.Mod(485868.249036+(1717915923.2178)*t, TURNAS) * DAS2R

	// Mean anomaly of the Sun.
	elp = math.Mod(1287104.79305+(129596581.0481)*t, TURNAS) * DAS2R

	// Mean argument of the latitude of the Moon.
	f = math.Mod(335779.526232+(1739527262.8478)*t, TURNAS) * DAS2R

	// Mean elongation of the Moon from the Sun.
	d = math.Mod(1072260.70369+(1602961601.2090)*t, TURNAS) * DAS2R

	// Mean longitude of the ascending node of the Moon.
	om = math.Mod(450160.398036+(-6962890.5431)*t, TURNAS) * DAS2R

	// Initialize the nutation values.
	dp = 0.0
	de = 0.0

	// Summation of luni-solar nutation series (smallest terms
	// first).
	for i = NLS - 1; i >= 0; i-- {

		// Argument and functions.
		arg = math.Mod(float64(x[i].nl)*el+
			float64(x[i].nlp)*elp+
			float64(x[i].nf)*f+
			float64(x[i].nd)*d+
